...
```
tanzu apps workload create node-express-test -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/node-express-test-source \
  --type web
```
...
```
tanzu apps workload get node-express-test
```
